...
namespace XAF_Bootstrap.Controls
{
    public class XafBootstrapNumericEdit : XafBootstrapBaseControl
    {
        ...
        public String InnerRender()
        {
            ...
            string val = String.Format(DisplayFormat, Value).Replace(System.Globalization.CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator, ".");
            ...
            {
                val = val.Replace(" ", "");
                String changeEvent = String.Format(@"onchange="";window.DataChanged=true; {0} """, String.Concat(CustomCallbackScript) == "" ? GetCallbackScript("'NewValue=' + $(this).val()") : CustomCallbackScript);                
                ...
                <div class=""input-group number-spinner"">
				    ...
				    </span>", ClientID, String.Concat(CustomCallbackScript) == "" ? GetCallbackScript("'NewValue=' + $(this).val()") : CustomCallbackScript);
                ...
                ", ClientID, String.Concat(CustomCallbackScript) == "" ? GetCallbackScript("'NewValue=' + $(this).val()") : CustomCallbackScript);    
            }
        ...
        protected override void OnCallback(DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            if (values.Count() > 1)
            {
                switch (values[0])
                {
                    case "NewValue":
                        decimal val = 0;
                        if (decimal.TryParse(String.Concat(values[1]).Replace(".", System.Globalization.CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator), out val))
                            Value = val;
                        break;
                }
            }            
