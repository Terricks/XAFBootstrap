...
<%@ Register Assembly="DevExpress.ExpressApp.Web.v15.2" Namespace="DevExpress.ExpressApp.Web.Templates"
    ...
    <div class="modal-header">        
        <button type="button" class="close" onclick="closeThisModalBefore()" aria-label="Close"><span aria-hidden="true" class="text-primary">&times;</span> </button>
        <button type="button" class="close" onclick="fullscreenThisModal()" aria-label="Maximize"><span class="text-primary glyphicon glyphicon-fullscreen" style="font-size: 60%; margin-top: 5px; margin-right: 5px"></span> </button>
    ...
    <div class="modal-body">
        <cc3:XafUpdatePanel ID="UPSAC" runat="server">
            <div class="row" id="upperActions" style="padding-bottom: 2px">
                <div class="col-sm-6">
...
<script>
    ...
    $(document).ready(function () {
        window.closeThisModalBefore = function () {
            if (!window.isModalClosing) {
                if ("<%= GetCloseActionName() %>" == "")
                    window.closeThisModal(true);
                else {
                    window.isModalClosing = true;
                    RaiseXafCallback(globalCallbackControl, "Dialog$PAC_Callback", "Action=<%= GetCloseActionName() %>", "", false);
                }
            }
        };
