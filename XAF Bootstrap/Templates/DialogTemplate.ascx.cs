...
namespace XAF_Bootstrap.Templates
{
    public partial class DialogTemplate : TemplateContent, ILookupPopupFrameTemplate, IXafPopupWindowControlContainer
    {
        ...
        {
            var ret = "";
            var act = PAC.actionObjects.FirstOrDefault(f => f.Key.ActionMeaning == DevExpress.ExpressApp.Actions.ActionMeaning.Cancel).Key;
            if (act == null)
                act = PAC.actionObjects.FirstOrDefault(f => f.Key.ActionMeaning == DevExpress.ExpressApp.Actions.ActionMeaning.Accept).Key;
            if (act != null)
                return act.Id;
